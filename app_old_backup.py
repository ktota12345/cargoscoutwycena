...
# Konfiguracja Backend Pricing API
...
# Funkcja do wywoływania Backend Pricing API
def call_backend_pricing_api(start_postal_code: str, end_postal_code: str, distance: float):
    """
    Wywołuje backend API do pobrania danych cenowych
    
    Args:
        start_postal_code: Kod pocztowy startu (np. "PL20")
        end_postal_code: Kod pocztowy celu (np. "DE49")
        distance: Dystans w km
    
    Returns:
        Dict z danymi z API lub None w przypadku błędu
    """
    if not BACKEND_API_URL or not BACKEND_API_KEY:
        print("⚠️ Brak konfiguracji Backend API - używam fallback do bazy danych")
        return None
    ...
    try:
        ...
        print(f"🌐 Wywołuję Backend API: {BACKEND_API_URL}")
        print(f"📤 Payload: {payload}")
        
        response = requests.post(BACKEND_API_URL, json=payload, headers=headers, timeout=30)
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                print(f"❌ API zwróciło błąd: {data.get('error')}")
                return None
        else:
        ...
        print(f"❌ Nie można połączyć się z Backend API: {BACKEND_API_URL}")
        return None
    except Exception as e:
        print(f"❌ Błąd wywołania Backend API: {e}")
        return None

# Funkcja przekształcająca dane z Backend API do formatu frontendu
def transform_api_response_to_frontend_format(api_data, distance, days):
    """
    Przekształca odpowiedź z Backend API do formatu oczekiwanego przez frontend
    
    Args:
        api_data: Dane z Backend API
        distance: Dystans w km
        days: Okres danych (7, 30, 90)
    
    Returns:
        Dict w formacie oczekiwanym przez frontend
    """
    if not api_data:
        return None
    ...
    # Mapowanie okresu na klucz w API (7d, 30d, 90d -> "30d" dla days=30)
    period_key = f"{days}d" if days in [7, 30, 90] else "30d"
    
    # Przygotuj oferty dla frontendu
    all_offers = []
    
    # Dane TimoCom
    timocom_has_data = data_sources.get('timocom', False)
    timocom_period_data = pricing.get('timocom', {}).get(period_key, {})
    
    if timocom_has_data and timocom_period_data:
        # TimoCom - trailer
        if timocom_period_data.get('avg_price_per_km', {}).get('trailer'):
            all_offers.append({
                'exchange': 'TimoCom',
                'vehicle_type': 'Naczepa',
                'rate_per_km': timocom_period_data['avg_price_per_km']['trailer'],
                'total_price': round(timocom_period_data['avg_price_per_km']['trailer'] * distance, 2),
                'currency': 'EUR',
                'has_data': True,
                'total_offers_sum': timocom_period_data.get('total_offers', 0),
                'records_count': timocom_period_data.get('days_with_data', 0)
            })
        
        # TimoCom - mega_trailer
        if timocom_period_data.get('avg_price_per_km', {}).get('mega_trailer'):
            all_offers.append({
                'exchange': 'TimoCom',
                'vehicle_type': 'Mega Naczepa',
                'rate_per_km': timocom_period_data['avg_price_per_km']['mega_trailer'],
                'total_price': round(timocom_period_data['avg_price_per_km']['mega_trailer'] * distance, 2),
                'currency': 'EUR',
                'has_data': True,
                'total_offers_sum': timocom_period_data.get('total_offers', 0),
                'records_count': timocom_period_data.get('days_with_data', 0)
    ...
    # Dane Trans.eu
    transeu_has_data = data_sources.get('transeu', False)
    transeu_period_data = pricing.get('transeu', {}).get(period_key, {})
    
    if transeu_has_data and transeu_period_data:
        if transeu_period_data.get('avg_price_per_km', {}).get('lorry'):
            all_offers.append({
                'exchange': 'Trans.eu',
                'vehicle_type': 'Ciężarówka',
                'rate_per_km': transeu_period_data['avg_price_per_km']['lorry'],
                'total_price': round(transeu_period_data['avg_price_per_km']['lorry'] * distance, 2),
                'currency': 'EUR',
                'has_data': True,
                'total_offers_sum': transeu_period_data.get('total_offers', 0),
                'records_count': transeu_period_data.get('days_with_data', 0)
            })
    else:
        # Brak danych Trans.eu
        all_offers.append({
            'exchange': 'Trans.eu',
            'rate_per_km': None,
            'total_price': None,
    ...
    # Oblicz średnie
    has_any_data = timocom_has_data or transeu_has_data
    
    if has_any_data:
        rates = [o['rate_per_km'] for o in all_offers if o.get('has_data') and o.get('rate_per_km')]
        totals = [o['total_price'] for o in all_offers if o.get('has_data') and o.get('total_price')]
        
        avg_rate = sum(rates) / len(rates) if rates else None
        avg_total = sum(totals) / len(totals) if totals else None
        
        # Określ źródło danych
        if timocom_has_data and transeu_has_data:
            data_source = 'both'
        elif timocom_has_data:
        ...
        return {
            ...
            'average_offers_per_day': None,  # API nie zwraca tej informacji
            'days': days,
            'data_source': data_source,
            'api_response': api_data  # Zachowaj pełną odpowiedź dla debugowania
        }
    else:
        return {
            'has_data': False,
...
def get_aggregated_exchange_data(start_region_id: int, end_region_id: int, distance: float, days: int = 7):
    ...
    # NAJPIERW: Spróbuj użyć Backend API (jeśli jest skonfigurowane)
    # Backend API potrzebuje kodów pocztowych, więc musimy je przekonwertować z region_id
    # Dla uproszczenia: jeśli Backend API jest dostępne, użyj go
    # Jeśli nie - fallback do bazy danych
    
    # TODO: Tutaj można dodać konwersję region_id -> postal_code jeśli potrzebna
    # Na razie fallback do staraszego kodu
...
# Funkcja przekształcająca dane historyczne z Backend API do formatu frontendu
def transform_historical_data_from_api(api_data, distance, days=7):
    """
    Przekształca dane historyczne z Backend API (FTL/LTL) do formatu frontendu
    ...
    # Sprawdź czy są dane FTL lub LTL
    ftl_data = historical.get('FTL')
    ltl_data = historical.get('LTL')
    ...
    # Przygotuj zlecenia z danych FTL i LTL
    orders = []
    
    # Dodaj przewoźników z FTL
    if ftl_data and ftl_data.get('top_carriers'):
        for carrier in ftl_data['top_carriers']:
            orders.append({
                'order_id': f'FTL-{carrier.get("carrier_id", "N/A")}',
                'date': 'Historical (180d)',
                'carrier': carrier.get('carrier_name', 'Unknown'),
                'rate_per_km': carrier.get('avg_carrier_price_per_km'),
                'total_price': carrier.get('avg_carrier_amount'),
                'currency': 'EUR',
                'type': 'FTL',
                'order_count': carrier.get('order_count', 0)
            })
    
    # Dodaj przewoźników z LTL
    if ltl_data and ltl_data.get('top_carriers'):
        for carrier in ltl_data['top_carriers']:
            orders.append({
                'order_id': f'LTL-{carrier.get("carrier_id", "N/A")}',
                'date': 'Historical (180d)',
                'carrier': carrier.get('carrier_name', 'Unknown'),
                'rate_per_km': carrier.get('avg_carrier_price_per_km'),
                'total_price': carrier.get('avg_carrier_amount'),
    ...
    # Oblicz średnie ze wszystkich danych
    all_rates = []
    all_totals = []
    
    if ftl_data and ftl_data.get('avg_price_per_km', {}).get('carrier'):
        all_rates.append(ftl_data['avg_price_per_km']['carrier'])
        all_totals.append(ftl_data.get('avg_amounts', {}).get('carrier', 0))
    
    if ltl_data and ltl_data.get('avg_price_per_km', {}).get('carrier'):
        all_rates.append(ltl_data['avg_price_per_km']['carrier'])
        all_totals.append(ltl_data.get('avg_amounts', {}).get('carrier', 0))
    
    avg_rate = sum(all_rates) / len(all_rates) if all_rates else None
    avg_total = sum(all_totals) / len(all_totals) if all_totals else None
    
    # Oblicz średnią liczbę zleceń
    total_orders = 0
    total_days = 0
    
    if ftl_data:
        total_orders += ftl_data.get('total_orders', 0)
        total_days = max(total_days, ftl_data.get('days_with_data', 0))
    
    if ltl_data:
        total_orders += ltl_data.get('total_orders', 0)
        total_days = max(total_days, ltl_data.get('days_with_data', 0))
    
    orders_per_day = round(total_orders / total_days, 1) if total_days > 0 else None
    ...
    return {
        ...
        'days': 180,  # API zwraca dane za 180 dni
        'ftl_data': ftl_data,
        'ltl_data': ltl_data
    }
...
def calculate_route():
    ...
    # PRIORYTET 1: Użyj Backend API (jeśli dostępne)
    # PRIORYTET 2: Użyj bazy danych (jeśli mamy region_id)
    # PRIORYTET 3: Użyj losowych danych (fallback)
    
    # Spróbuj użyć Backend API
    backend_api_data = None
    if BACKEND_API_URL and BACKEND_API_KEY and start_location and end_location:
        print(f"🌐 Próba użycia Backend API dla {start_location} -> {end_location}")
        backend_api_data = call_backend_pricing_api(start_location, end_location, distance)
    
    if backend_api_data:
        # Użyj danych z Backend API
        print(f"✅ Używam danych z Backend API")
        exchange_data = transform_api_response_to_frontend_format(backend_api_data, distance, days)
        exchange_data_7 = transform_api_response_to_frontend_format(backend_api_data, distance, 7)
        exchange_data_30 = transform_api_response_to_frontend_format(backend_api_data, distance, 30)
        exchange_data_90 = transform_api_response_to_frontend_format(backend_api_data, distance, 90)
    elif start_region_id and end_region_id:
        # Fallback do bazy danych
        print(f"📊 Fallback: Pobieranie danych z bazy dla regionów: {start_region_id} -> {end_region_id}")
        ...
        print("⚠ Brak Backend API i ID regionów - używam losowych danych")
    ...
    # Dane historyczne - użyj API jeśli dostępne
    if backend_api_data:
        historical_data = transform_historical_data_from_api(backend_api_data, distance, days)
        historical_data_7 = transform_historical_data_from_api(backend_api_data, distance, 7)
        historical_data_30 = transform_historical_data_from_api(backend_api_data, distance, 30)
        historical_data_90 = transform_historical_data_from_api(backend_api_data, distance, 90)
    else:
